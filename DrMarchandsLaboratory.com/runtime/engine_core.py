...
def shelve_dead_letter(filename, error_type, error_msg):
    ...
    try:
        ...
        print(f"[⚠️ ISOLATED] Signal moved to dead-letter storage: {error_id} | Reason: {error_msg}")
    except Exception as exc:
        print(f"[⚠️ ERROR] Failed to isolate {filename}: {exc}")
...
def process_signal(filename):
    ...
    try:
        ...
        return shelve_dead_letter(filename, "ingest_fail", exc)

    try:
        result = registry.execute_instinct(payload)
    except Exception as exc:
        return shelve_dead_letter(filename, "actuator_crash", exc)

    receipt_name = filename.replace(".json", ".receipt.json")
    with open(os.path.join(RECEIPT_DIR, receipt_name), "w") as f:
        json.dump(result, f, indent=2)
    print(f"[✓ PROCESSED] Receipt written: {receipt_name}")


def engine_loop():
    print("[⚙︎ NFE] Runtime core online. Monitoring configured signal directory.")
    ...
    while True:
        registry.refresh()
        signals = sorted(
            f for f in os.listdir(CLOUD_SIGNAL_DIR) if f.endswith(".json")
        )
        ...
        new_signals = [signal for signal in signals if signal > last_file]
        ...
        buckets = [
            new_signals[index:index + MAX_BATCH_SIZE]
            for index in range(0, len(new_signals), MAX_BATCH_SIZE)
        ]
